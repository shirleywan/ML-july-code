...
def clean_info(info):
    replace_str = (('\n', ''), ('\r', ''), (',', '，'), ('[表情]', ''))#去掉回车，空格，表情； (',', '，')半角的替换成全角的
    for rs in replace_str:
        info = info.replace(rs[0], rs[1])#直接替换

    at_pattern = re.compile(r'(@.* )') #@人的形式，一定是“@+人名+空格”的形式，空格表示@结束，是一种模式；清掉这种模式
    #也有人会在后面@，那么@人格式的空格会在取出前后空格时被去掉；
    at = re.findall(pattern=at_pattern, string=info)
    for a in at:
        info = info.replace(a, '')
    idx = info.find('@') #看能不能再发现@
    if idx != -1:
        info = info[:idx] #只保留@前面的数据
    return info


def regularize_data(file_name): #一个人说的话里可能有硬回车，所以不能把每作为一个文档；需要对文档修改；
    time_pattern = re.compile(r'\d{4}-\d{2}-\d{2} \d{1,2}:\d{1,2}:\d{1,2}')#d{4}识别4位，年份；\d{4}-\d{2}-\d{2}识别时间：年-月-日；d{1,2}1位或两位；
    qq_pattern1 = re.compile(r'([1-9]\d{4,15})')    # QQ号最小是10000
    qq_pattern2 = re.compile(r'(\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*)')#邮箱注册的qq号，re模块
    f = open(file_name)
    ...
    for line in f:
        ...
        if line:
            ...
            qq2 = re.findall(pattern=qq_pattern2, string=line)#如果能找到一个时间和一个qq号（两种注册都可以），就认为是一个title；
            if (len(t) >= 1) and ((len(qq1) >= 1) or (len(qq2) >= 1)):
                if info: #匹配好，把上一个人的信息输入到结果中
                    info = clean_info(info)#可能是表情，图片[图片]，或是空；去掉这些
                    if info:
                        info = '%s,%s,%s\n' % (qq, chat_time, info)
                        f_output.write(info)
                        info = ''
                if len(qq1) >= 1:#qq1模式匹配
                    qq = qq1[0]
                else:
                    qq = qq2[0][0]#找到最早匹配到的邮箱
                chat_time = t[0]#匹配时间
            else:
                info += line#有文本，但是无时间和qq号，那么就是上一个人说话没说完，打的硬回车；继续作为信息本身
    f.close()
    f_output.close()
...
def export_perplexity1(corpus_tfidf, dictionary, corpus):#可以算困惑度
    lp1 = []
    lp2 = []
    ...
    for t in topic_nums:
        ...
        lp = model.log_perplexity(corpus)#输出log困惑度，每个主题都算困惑度（用原始语料，或tfidf做），选择拐点位置做主题k的个数
        print 't = ', t,
