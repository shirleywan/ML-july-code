...
def Normalize(features):
    ...
    for i in range(len(features)):
        features[i]=features[i]/(np.sqrt(features[i].dot(features[i])) + eps)
    return features
...
def query(extraFEATs,galleryFEATs,queryFEATs,extraList,galleryList,queryList,saveDir,dims,isNorm,isSig,isHash,distance,topN,proc_max):
    ...
    print ' - Combined keys:',keyFeatures.shape[0],keyFeatures.shape[1]
    queryFile = np.fromfile(queryFEATs, dtype = 'float32')
    queryFeatures = queryFile.reshape((len(queryFile)/dims, dims))
    print ' - Query loaded:',queryFeatures.shape[0],queryFeatures.shape[1]
...
def query_flip(extraFEATs,galleryFEATs,queryFEATs,extraList,galleryList,queryList,saveDir,dims,isNorm,isSig,isHash,distance,topN,proc_max,extraFlipFEATs,galleryFlipFEATs,queryFlipFEATs):
    #print len([1, 2, 3])
    print '[ Load features ]'
    extras = np.fromfile(extraFEATs, dtype = 'float32') + np.fromfile(extraFlipFEATs, dtype = 'float32')
    print ' - Extra loaded:',len(extras)/dims
    gallerys = np.fromfile(galleryFEATs, dtype = 'float32') + np.fromfile(galleryFlipFEATs, dtype = 'float32')
    print ' - Gallery loaded:',len(gallerys)/dims
    keys= np.concatenate((gallerys,extras))
    global keyFeatures
    keyFeatures = keys.reshape((len(keys)/dims, dims))
    print ' - Combined keys:',keyFeatures.shape[0]
    queryFile = np.fromfile(queryFEATs, dtype = 'float32') + np.fromfile(queryFlipFEATs, dtype = 'float32')
    queryFeatures = queryFile.reshape((len(queryFile)/dims, dims))
...
if __name__=='__main__':
   ...
   if len(sys.argv) == 15:
       ...
       query_flip(sys.argv[1],sys.argv[2],sys.argv[3],sys.argv[4],sys.argv[5],sys.argv[6],sys.argv[7],int(sys.argv[8]),int(sys.argv[9]),int(sys.argv[10]),int(sys.argv[11]),sys.argv[12],int(sys.argv[13]),int(sys.argv[14]),sys.argv[15],sys.argv[16],sys.argv[17])
