...
if __name__ == "__main__":
    warnings.filterwarnings(action='ignore', category=UndefinedMetricWarning)#忽略0*0的情况
    ...
    clfs = [svm.SVC(C=1, kernel='linear'),
           svm.SVC(C=1, kernel='linear', class_weight={-1: 1, 1: 50}), #class_weight={-1: 1, 1: 50}，给权值，负例给1，正例给50
           svm.SVC(C=0.8, kernel='rbf', gamma=0.5, class_weight={-1: 1, 1: 2}),#正例给2
           svm.SVC(C=0.8, kernel='rbf', gamma=0.5, class_weight={-1: 1, 1: 10})] #正例给10
    ...
    for i, clf in enumerate(clfs):
        ...
        print i+1, '次：'
        print 'accuracy：\t', accuracy_score(y, y_hat) #算准确率
    ...
    #根据每幅图的4个评判标准：
        # 1-precision很高，但是recall不好；因为红色的点没有被分在红色区域，而是在绿色区域内，因此被判定为负例，是FN；
        # 2-recall很高，但是precision较低；许多绿色点被判定为正例，是FP；
        #3、4的结果就会好很多
